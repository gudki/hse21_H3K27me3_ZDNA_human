setwd('/Users/mark/Desktop/bio/git/hse21_H3K27me3_ZDNA_human/src')
source('lib.R')
...
peaks <- toGRanges(paste0(DATA_DIR, 'zhunt.intersect.bed'), format="BED")
peaks[1:2]
...
write.table(anno_df, file=paste0(DATA_DIR, 'zhunt.intersect.genes.txt'),
            col.names = TRUE, row.names = FALSE, sep = '\t', quote = FALSE)

uniq_genes_df <- unique(anno_df['symbol'])
write.table(uniq_genes_df, file=paste0(DATA_DIR, 'zhunt.intersect.genes_uniq.txt'),
